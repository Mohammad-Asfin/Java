...
## 🔗 JDBC — Java Database Connectivity (Learning Module)

### 📖 Introduction to JDBC

**What is JDBC?**
Java Database Connectivity (JDBC) is a standard Java API used to connect Java applications with relational databases (like MySQL, PostgreSQL, Oracle). It allows you to execute SQL queries, update records, and retrieve results directly from Java code.

**Why is JDBC Needed & What Problem Does It Solve?**
Different databases have different proprietary communication protocols. Without JDBC, a Java developer would have to write specific code for PostgreSQL, different code for MySQL, and different code for Oracle. JDBC solves this by providing a **unified set of interfaces**. You write your Java code once using JDBC APIs, and the database-specific "JDBC Driver" translates it for the actual database.

**JDBC Workflow & Architecture**
The complete workflow from your Java app to the database and back:
1. **Java Application**: Your code requests a database operation.
2. **JDBC API**: Standard classes/interfaces (`Connection`, `Statement`, `ResultSet`).
3. **JDBC Driver**: Translates standard API calls into database-specific protocol.
4. **Database**: Executes the SQL Query (e.g., MySQL, PostgreSQL).
5. **Result**: The database sends the raw result back to the Driver.
6. **Java Application**: Your code processes the data using a `ResultSet`.
...
### 🗺️ JDBC Learning Path & Practical Examples

Follow this progression to learn JDBC step-by-step. Each topic links directly to a practical, runnable Java example in this repository.

| # | JDBC Topic | What You'll Learn | Example Code |
| :--- | :--- | :--- | :--- |
| 1 | **Connection** | Load Driver & Connect Java to DB | [DemoJdbc.java](Connecting%20Java%20and%20DB/src/DemoJdbc.java) |
| 2 | **Statement (Insert)** | Execute basic SQL INSERT | [LaunchApp1.java](JDBC/JDBC/src/JDBCTest/LaunchApp1.java) |
| 3 | **Statement (Update)** | Modify existing records | [DemoJdbc.java](JDBC_Update/src/DemoJdbc.java) |
| 4 | **Statement (Delete)** | Remove records from the DB | [DemoJdbc.java](JDBC_Delete/src/DemoJdbc.java) |
| 5 | **CRUD Operations** | Complete CREATE, READ, UPDATE, DELETE flow | [DemoJdbc.java](Crud%20operations/src/DemoJdbc.java) |
| 6 | **ResultSet (Fetch One)** | Read a specific record using `executeQuery()` | [DemoJdbc.java](Execute%20and%20Process/src/DemoJdbc.java) |
| 7 | **ResultSet (Fetch All)** | Iterate through multiple records with `rs.next()` | [DemoJdbc.java](Fetching%20all%20records/src/DemoJdbc.java) |
| 8 | **Problems with Statement** | Understand SQL Injection risks & string concatenation | [DemoJdbc.java](Problems%20with%20Statement/src/DemoJdbc.java) |
| 9 | **PreparedStatement** | Secure, parameterized SQL execution using `?` | [DemoJdbc.java](PreparedStatement/src/DemoJdbc.java) |
| 10 | **Exception Handling** | Gracefully handle `SQLException` & catch DB errors | [DemoJdbc.java](JDBC_Exception_Handling/src/DemoJdbc.java) |
| 11 | **try-with-resources** | Auto-close Connection, Statement, and ResultSet | [DemoJdbc.java](JDBC_Try_With_Resources/src/DemoJdbc.java) |

---

### 🗄️ Standard Database Schema
All examples in this repository use the following simple `student` database schema. You can run this in your local database before trying the code:
...
### ⚖️ Statement vs. PreparedStatement (Before & After)

#### Before: Using `Statement` (Not Recommended)
When using a normal `Statement`, you must use string concatenation to pass dynamic variables into the SQL string.
```java
Statement st = con.createStatement();
String sql = "INSERT INTO student (id, name, age) VALUES (" + sid + ", '" + sname + "', " + age + ")";
st.executeUpdate(sql);
```
**Why this is bad**: 
1. Very hard to read (too many quotes `'" + + "'`).
2. Extremely vulnerable to **SQL Injection**. If `sname` is `"Max', 22); DROP TABLE student; --"`, the database gets destroyed!

#### After: Using `PreparedStatement` (Recommended)
`PreparedStatement` pre-compiles the query and uses `?` placeholders. The variables are safely bound later.
```java
...
| :--- | :--- | :--- |
| **Parameters** | Hardcoded or String Concatenation | Dynamic `?` placeholders |
| **Security** | ❌ Vulnerable to SQL Injection | ✅ Secure against SQL Injection |
| **Performance**| Slower (compiled every time) | Faster (Pre-compiled once) |
| **Readability**| Messy (lots of quotes and pluses) | Clean and easy to maintain |
| **Usage** | Static queries with no dynamic data | Queries with user inputs |

---

### ⚠️ Common Mistakes & Troubleshooting

If your JDBC program fails, check these common errors:
* **`java.sql.SQLException: No suitable driver found for jdbc:postgresql...`**: You forgot to add the database driver JAR file to your project build path/classpath.
* **`Connection refused`**: Your database server (MySQL/PostgreSQL) is not running, or you used the wrong port (e.g., 5432 vs 3306).
* **`Access denied for user`**: Wrong username or password in `DriverManager.getConnection()`.
* **`Unknown database 'java_jdbc'`**: You forgot to run the `CREATE DATABASE java_jdbc;` SQL command first.
* **`ResultSet closed` or `Before start of result set`**: You forgot to call `rs.next()` before trying to read data with `rs.getString()`.
* **Memory Leaks**: Forgetting to call `con.close()`, `st.close()`, and `rs.close()`. Always use a `finally` block or `try-with-resources`.

---

### 🏋️ Practice Exercises

Once you have studied the examples, try building these yourself from scratch:

**Beginner:**
1. Connect to the database and create a new table called `employees`.
2. Insert 5 employees into the table.
3. Fetch and print all employees using a `ResultSet`.
4. Update the salary of a specific employee by ID.
5. Delete an employee from the table.

**Intermediate:**
6. Implement a complete CRUD application using `PreparedStatement` instead of `Statement`.
7. Use `Scanner` to ask the user for an ID, then search and display that specific student.
8. Use `ResultSetMetaData` to print out the column names of a table dynamically.
