...
### What is JDBC?
Java Database Connectivity (JDBC) is a Java API that manages connecting to a database, issuing queries and commands, and handling result sets obtained from the database. It provides a standard interface for Java applications to interact with relational databases.

### Why JDBC is used
JDBC provides a unified set of APIs that allow Java applications to remain database-independent. Instead of writing custom code for MySQL, PostgreSQL, or Oracle, developers write standard JDBC code, and the specific JDBC driver handles the database-specific communication.

### JDBC Learning Flow
The standard process to execute database operations using JDBC involves the following steps:
`Database → JDBC Driver → Connection → Statement/PreparedStatement → Execute SQL → ResultSet → Process Data → Close Resources`
...
### Key JDBC Components
1. **JDBC Driver**: A software component enabling Java applications to interact with a specific database (e.g., PostgreSQL driver).
2. **Connection**: Represents a session with a specific database (`java.sql.Connection`).
3. **Statement**: Used to execute static SQL statements and return the results it produces (`java.sql.Statement`).
4. **PreparedStatement**: An object that represents a precompiled SQL statement, used to execute parameterized queries efficiently (`java.sql.PreparedStatement`).
5. **ResultSet**: A table of data representing a database result set, which is usually generated by executing a statement that queries the database (`java.sql.ResultSet`).

### 📚 JDBC Examples in This Repository

| Topic | Description | Example Code |
| :--- | :--- | :--- |
| **Connection** | Establishing a connection to PostgreSQL | [DemoJdbc.java](Connecting%20Java%20and%20DB/src/DemoJdbc.java) |
| **Statement** | Executing basic SQL statements (String concatenation) | [DemoJdbc.java](Problems%20with%20Statement/src/DemoJdbc.java) |
| **PreparedStatement**| Parameterized SQL queries using `?` placeholders | [DemoJdbc.java](PreparedStatement/src/DemoJdbc.java) |
| **ResultSet** | Reading query results (fetching specific or all records) | [DemoJdbc.java](Fetching%20all%20records/src/DemoJdbc.java) |
| **CRUD** | Performing INSERT, UPDATE, DELETE operations | [DemoJdbc.java](Crud%20operations/src/DemoJdbc.java) |

### 🛠️ Core Concepts Implemented

#### 1. Establishing a Database Connection
To connect to a database, the `DriverManager.getConnection()` method is used. It requires the connection URL, username, and password.
```java
String url = "jdbc:postgresql://localhost:5432/demo";
Connection con = DriverManager.getConnection(url, "postgres", "password");
```
*See example in: [Connecting Java and DB](Connecting%20Java%20and%20DB/src/DemoJdbc.java)*

#### 2. Executing CRUD Operations
Using the `Statement.execute()` method, we can perform basic Data Manipulation Language (DML) commands:
*   **INSERT**: `insert into student values (5, 'John', 48)`
*   **UPDATE**: `update student set sname = 'Max' where sid = 5`
*   **DELETE**: `delete from student where sid = 5`

*See example in: [Crud operations](Crud%20operations/src/DemoJdbc.java)*

#### 3. Problems with `Statement`
Using plain `Statement` for dynamic data requires string concatenation:
```java
String sql = "insert into student values ("+sid+",'"+sname+"',"+marks+")";
```
**Limitations**: 
* Vulnerable to SQL Injection attacks.
* Hard to read and maintain due to complex quotes and plus signs.
* Less efficient for repeated execution.
*See example in: [Problems with Statement](Problems%20with%20Statement/src/DemoJdbc.java)*

#### 4. The Solution: `PreparedStatement`
`PreparedStatement` pre-compiles the SQL and uses `?` placeholders, making it secure and efficient.
```java
...
st.execute();
```
*See example in: [PreparedStatement](PreparedStatement/src/DemoJdbc.java)*
...
#### 6. ResultSets and Retrieving Records
The `executeQuery()` method returns a `ResultSet`. You can iterate through the results using `rs.next()`.
```java
...
```
*See examples in: [Execute and Process](Execute%20and%20Process/src/DemoJdbc.java) | [Fetching all records](Fetching%20all%20records/src/DemoJdbc.java)*

#### 7. JDBC Exception Handling & Resource Management
JDBC operations throw `SQLException`, which must be caught or declared using `throws Exception`.
After operations, it is critical to close resources (Connection, Statement, ResultSet) using `con.close()` to prevent memory leaks and database lockups.
